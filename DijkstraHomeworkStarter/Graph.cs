...
namespace DijkstraHomeworkStarter
{
    class Graph
    {
        ...
        public void FindAndHighlightShortestPaths(Vertex v)
        {
            ...
            {
                if ((vertex.Permanent) && (vertex.PreviousVertex != null))
                {
                    HighlightEdge(vertNameToIndex[vertex.Name], vertNameToIndex[vertex.PreviousVertex.Name]);
                }
            }
        }
                ...
                {                                                                           // wasn't found before
                    if (adjMatrix[indexOfVertexInList, incrementor] != 0)
                    {
                            ...
                            (vertices[incrementor] != sourceVertex))
                        {
                            ...
                            else if (vertices[incrementor].Distance < priorityQueue.VertexFromQueue(vertices[incrementor]).Distance)
                            {
                                priorityQueue.ReplaceVertex(vertices[incrementor]);
                            }
                        }
                    }
